...
// (JWT) token verification for ADMIN
verifyToken.verifyAdminToken = function(token,res,callback){
  if(token !== undefined && token !== null){
    //let token = cookie.split('=');
    let data = jwt.verify(token, config.secret_admin, function(err,authData){
      if(err){
        res.sendStatus(403);
      }
      else{
    ...
    throw Error('Access denied. Please login as Admin to continue');
  }
}
